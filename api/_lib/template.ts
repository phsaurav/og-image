...
export function getHtml(parsedReq: ParsedRequest) {
	...
	const title =
		text === "both" ? "A Teacher !!!" : text === "male" ? "Male Teacher" : "Female Tutor";
			...
			<div class="group-div">
				<div class="group">
					<h3 class="group-title">Group: ${
						group && group.charAt(0).toUpperCase() + group.substr(1).toLowerCase()
					}</h3>
					<h3 class="group-shadow">Group: ${
						group && group.charAt(0).toUpperCase() + group.substr(1).toLowerCase()
					}</h3>
				</div>
			</div>
				...
				${
					subjects && subjects.length > 5
						? subjects
						  ...
						  ` <div class="subject" style="margin-top:20px; text-align:center">+${
								subjects.length - 4
						  } More Subjects</div>`
